...
# gtf <- "./Data/gencode.v35.annotation.gtf.gz"
...
sam CIGAR strings in the mitomap2 alignments using a custom script.

### Illumina samples

The two EDTA blood samples sequenced using Illumina NGS technology were aligned
against the whole genome using the GATK recommended guidelines, including the
...
Metadata %>%
    ...
    kable(caption = "Sample overview") %>% kable_styling(bootstrap_options = "striped", full_width = FALSE)
```

NOTE: sample from PFC is from a different individual.
...
depthFiles1 <- paste0("../results/depth/", filter(Metadata, platform == "ont")$sample_id, "_chrM.txt")
names(depthFiles1) <- filter(Metadata, platform == "ont")$sample_id
#all(file.exists(depthFiles1))

depthFiles2 <- paste0("../results/depth/", filter(Metadata, platform == "ont")$sample_id, "_chrM_shifted.txt")
names(depthFiles2) <- filter(Metadata, platform == "ont")$sample_id
#all(file.exists(depthFiles2))

depthFiles22 <- paste0("../results/depth/", filter(Metadata, platform == "ont")$sample_id, "_chr22.txt")
names(depthFiles22) <- filter(Metadata, platform == "ont")$sample_id
#all(file.exists(depthFiles22))


## COVERAGE / SUMMARY STATS
Cov1 <- lapply(names(depthFiles1), function(sid) {
...
Cov2 <- lapply(names(depthFiles2), function(sid) {
    read_tsv(depthFiles1[sid], col_names = FALSE, col_types = "cii") %>%
        mutate(sample_id = sid) %>%
        select(Pos = X2, Cov = X3, sample_id) %>%
...
Cov <- bind_rows(Cov1, Cov2) %>% arrange(Pos)
rm(Cov1, Cov2)
invisible(gc())

Labels <- left_join(Cov, Metadata, by = join_by(sample_id)) %>%
    group_by(tissue) %>%
    filter(Pos == 16569) %>%
    ungroup()
Plot1 <- left_join(Cov, Metadata, by = join_by(sample_id)) %>%
    ...
    geom_text_repel(data = Labels, aes(x = Pos, y = Cov, label = tissue),
...
# invisible(svg("./Figs/coverage_ONT.svg", height = 6, width = 12))
# Plot1
# invisible(dev.off())
...
with the nuclear genome. Based on the region chr22:20,000,000-20,100,000, the
nuclear genome has a coverage at least 2 orders of magnitude lower.

In addition, the nuclear coverage is much more similar across different tissues
than the coverage for the mitochondrial genome, which exhibits very large
differences (e.g., ~400X in EDTA blood vs ~200,000X in heart tissue).

The following table display summary statistics for both the rCRS coverage (top)
and a small region in chr22 (chr22:20,000,000-20,100,000, bottom).
...
summTable <- left_join(Cov, Metadata, by = join_by(sample_id)) %>%
    ...
    summarise(mean_cov = round(mean(Cov)),
              ...
              perc_5 = round(quantile(Cov, 0.05)),
              perc_25 = round(quantile(Cov, 0.25)),
              median_cov = round(median(Cov)), 
              perc_75 = round(quantile(Cov, 0.75)),
              perc_95 = round(quantile(Cov, 0.95))) %>%
    ungroup() %>%
    left_join(Metadata, by = join_by(sample_id)) %>%
    select(tissue, individual_id, sample_id, mean_cov, stdev_cov, median_cov, starts_with("perc"))
...
This data can be used to estimate the a relative copy number across tissue, by
normalising mtDNA coverage by nuclear coverage. This represents only a relative
...
```{r}
summTable_ont <- summTable %>% select(tissue, individual_id, sample_id, median_cov_MT = median_cov) %>%
    full_join(summTable22 %>% select(tissue, individual_id, sample_id, median_cov_chr22 = median_cov)) %>%
    mutate(relative_copynumber = round(median_cov_MT/median_cov_chr22))
summTable_ont %>% 
...
```{r}
depthFiles_wgs_1 <- paste0("../results/depth/", filter(Metadata, platform == "illumina")$sample_id, "_chrM.txt")
names(depthFiles_wgs_1) <- filter(Metadata, platform == "illumina")$sample_id
#all(file.exists(depthFiles_wgs_1))

depthFiles_wgs_2 <- paste0("../results/depth/", filter(Metadata, platform == "illumina")$sample_id, "_chrM_shifted.txt")
names(depthFiles_wgs_2) <- filter(Metadata, platform == "illumina")$sample_id
#all(file.exists(depthFiles_wgs_2))

depthFiles_wgs_22 <- paste0("../results/depth/", filter(Metadata, platform == "illumina")$sample_id, "_chr22.txt")
...
Cov_wgs <- bind_rows(Cov_wgs_1, Cov_wgs_2) %>% arrange(Pos)
rm(Cov_wgs_1, Cov_wgs_2)
invisible(gc())
...
summTable_illumina <- summTable_wgs %>% select(tissue, individual_id, sample_id, median_cov_MT = median_cov) %>%
    full_join(summTable_wgs_22 %>% select(tissue, individual_id, sample_id, median_cov_chr22 = median_cov)) %>%
    mutate(relative_copynumber = round(median_cov_MT/median_cov_chr22))

bind_rows(summTable_illumina, summTable_ont) %>% filter(tissue == "edta_blood") %>%
    left_join(Metadata %>% select(sample_id, platform)) %>%
    select(individual_id, platform, cov_MT = median_cov_MT, cov_chr22 = median_cov_chr22, copy_nr = relative_copynumber) %>%
    pivot_wider(names_from = platform, values_from = c(cov_MT, cov_chr22, copy_nr)) %>%
    kable(caption = "Relative mtDNA copy number in 2 EDTA blood samples (ONT and Illumina)") %>%
    kable_styling(bootstrap_options = "striped", full_width = FALSE)
```
...
loads across tissues. Here, we define "mutational load" as the number of
single-base nucletide mismatches in a fixed length, e.g., 10 SNVs per 100
bases.

### Sequencing error

First, we can compare the error rate of ONT and Illumina sequencing platfoms by
leveraging the EDTA blood samples from the same 2 individuals. The sequencing
error rate is here calculated using the chr22:20,000,000-20,100,000 region, which
should only exhibit diploid genotypes. Only positions with a coverage within two
standard deviations of the mean coverage for the sample are considered. We then
calculate the number of erroneous calls as the bases that are different from
the genotype.

The following plots represent this error along the region studied for the four
samples (top samples, ONT, bottom samples, Illumina). Missing regions
correspond to low coverage. The discrete nature of the coverage results
in the "steps" seen in the plots.

In the case of the Illumina samples, some small regions seem to be problematic
for both samples, suggesting sequence composition playing a role.

```{r, fig.height=6, fig.width=9}
perNucFiles <- paste0("../results/depth/", filter(Metadata, tissue == "edta_blood")$sample_id, "_per_nuc_depth_chr22.txt")
...
perNucFiles2 <- paste0("../results/depth/", filter(Metadata, platform == "ont", individual_id %in% c("a","b"))$sample_id, "_per_nuc_depth_chrM.txt")
...
```{r, fig.height=4, fig.width=7}
snvsPerReadFiles <- paste0("../results/snvs_per_read/", filter(Metadata, platform == "ont", individual_id %in% c("a","b"))$sample_id, ".tsv")
...
Plot8 <- snvsPerRead_chr22 %>%
    filter(M > -10) %>%
    ggplot() +
    geom_density(aes(x = M, colour = sample_id))
    #geom_vline(xintercept = -2.8, linetype = 2)
...
## }
## 
## # From chatGPT, which is smarter than me
## # UNTESTED
...
## 
## 
## #svg("./Figs/coverage_ONT.svg", height = 6, width = 12)
## invisible(png("./Figs/coverage_ONT.png", height = 1000, width = 2000, res = 150))
## left_join(Cov, Metadata, by = join_by(sample_id)) %>%
##     filter(tissue != "PFC") %>%
##     ggplot(aes(x = Pos, y = Cov, colour = tissue)) +
##     #geom_point() +
...
##     filter(tissue != "PFC") %>%
##     group_by(tissue) %>%
##     summarise(perc_5 = round(quantile(Cov, 0.05)),
##               perc_25 = round(quantile(Cov, 0.25)),
##               median_cov = round(median(Cov)), 
##               perc_75 = round(quantile(Cov, 0.75)),
##               perc_95 = round(quantile(Cov, 0.95)))
## write.table(summTable, "./Tables/coverage_table.txt", sep = ",", quote = FALSE, row.names = FALSE)
## ##sjPlot::tab_mod(variable, file = "table.docx")
## 
## 
## 
## #########################

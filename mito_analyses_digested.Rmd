...
We can formally test whether these differences are significant.
...
Del %>%
    ...
    filter(!is.infinite(log10_del_size)) %>%
    compare_means(formula = log10_del_size ~ tissue, data = ., comparisons = my_comparisons, label = "p.signif") %>%
...
lm(formula = log10_del_size ~ tissue + read_length, data = 
    Del %>%
        ...
        filter(!is.infinite(log10_del_size))) %>%
    broom::tidy() %>%
    mutate(estimate = formatC(estimate, digits = 3, format = "g", flag = "#"),
           std.error = formatC(std.error, digits = 3, format = "g", flag = "#"),
           statistic = formatC(statistic, digits = 3, format = "g", flag = "#"),
           p.value = formatC(p.value, digits = 3, format = "g", flag = "#")) %>%
